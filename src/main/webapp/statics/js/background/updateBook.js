...
$(function () {
    ...
    let name1 = bookName.val();
    let author1 = bookAuthor.val();
    let type1 = bookType.val();
    let publishDate1 = bookPublishDate.val();
    let description1 = bookDescription.val();
    $('.backBtn').click(function () {
        window.location.href = path + "/background/book.do?method=bookQuery";
    });
    bookName.on('focus', function () {
        validateTip(bookName.next(), {"color":"red", "margin-left": "10px"}, "请输入正确的书籍名称", false);
    }).on('blur', function () {
        name = bookName.val();
        if(name.length === 0){
    ...
    bookAuthor.on('focus', function () {
        validateTip(bookAuthor.next(), {"color":"red", "margin-left": "10px"}, "请输入正确的作者名称", false);
    }).on('blur', function () {
    ...
    bookType.on('focus', function () {
        validateTip(bookType.next(), {"color":"red", "margin-left": "10px"}, "请输入正确的书籍类型", false);
    }).on('blur', function () {
        type = bookType.val();
        if(type.length === 0){
    ...
    bookPublishDate.on('focus', function () {
        validateTip(bookPublishDate.next(), {"color":"red", "margin-left": "10px"}, "请输入正确的出版时间", false);
    }).on('blur', function () {
    ...
    bookDescription.on('focus', function () {
        validateTip(bookDescription.next(), {"color":"red", "margin-left": "10px"}, "请输入正确的书籍简介", false);
    }).on('blur', function () {
    ...
    $('.updateBtn').on('click', function () {
        if((name1 === name || bookName.attr("validateStatus") === "true")
            && (author === author1 || bookAuthor.attr("validateStatus") === "true")
            && (type === type1 || bookType.attr("validateStatus") === "true")
            && (publishDate === publishDate1 || bookPublishDate.attr("validateStatus") === "true")
            && (description === description1 || bookDescription.attr("validateStatus") === "true")){
